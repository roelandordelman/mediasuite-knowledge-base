...
- [x] **Wiki graph enrichment** — broadcaster–production links extracted via `title_to_type` at RDF-build time (7,013 `beng:broadcaster` triples); person active periods enriched via Wikidata P1741 → birth/death dates (`harvest/enrich_wikidata.py`): 147/260 persons enriched, 2,146 persons now have a structured active period. Note: Fuseki not yet reloaded after broadcaster fix — `build_rdf.py` + `load_fuseki.py` still needed to activate `productions_by_broadcaster` query.
...
- [x] **Wiki path evaluation** (`eval_wiki.py`) — 7/7 questions passing (100%): period SPARQL (1970s, 1980s), function+period SPARQL (presentatoren jaren zeventig), biographical semantic (Mies Bouwman, Joris Ivens, Frits Thors), no-attribution check for documentation questions. Known issue: LLM selectively presents SPARQL result lists rather than showing all 50 entries as instructed — routing and retrieval are correct, this is an LLM compliance gap.
- [ ] **Rebuild wiki Fuseki graph** — run `build_rdf.py` + `load_fuseki.py` in mediasuite-wiki-agent to activate broadcaster triples; then annotate VPRO productions eval question.
