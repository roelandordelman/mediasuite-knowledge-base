...
- [x] **Wiki graph enrichment** — broadcaster–production links extracted via `title_to_type` at RDF-build time (7,013 `beng:broadcaster` triples); person active periods enriched via Wikidata P1741 → birth/death dates (`harvest/enrich_wikidata.py`): 147/260 persons enriched, 2,146 persons now have a structured active period.
