...
- [x] Entity-URI filter: SPARQL result URIs fed back into ChromaDB for targeted chunk retrieval; `entity_description` subject URI now explicitly added to filter (was previously missing — URI is SPARQL subject, not a returned value)
- [x] Named-entity detection: `QueryIndex._detect_named_entity()` checks tool/collection names verbatim in question text (longest match first); fires `entity_description` directly without relying on trigger question similarity — rule: "named tool/collection in query → always use entity_description + entity_uri filter"
...
- [x] **Wiki path** — third parallel retrieval path: chatbot calls [mediasuite-wiki-agent](https://github.com/roelandordelman/mediasuite-wiki-agent) REST API `POST /ask` (port 8002); dual-path retrieval inside the wiki agent: keyword-routed SPARQL queries (period, function, genre, broadcaster) + Milvus semantic search (24,104 articles, multilingual-e5-large-instruct); results merged by wiki agent and returned as one context block; semantic results filtered at cosine similarity ≥ 0.70; degrades gracefully when wiki service is down. Enables structured enumeration questions ("who was active between 1980 and 1990?") as well as open biographical questions. Fuseki is shared: wiki loaded as a separate dataset (`wiki`).
- [x] **Wiki graph enrichment** — broadcaster–production links extracted via `title_to_type` at RDF-build time (7,013 `beng:broadcaster` triples); person active periods enriched via Wikidata P1741 → birth/death dates (`harvest/enrich_wikidata.py`): 147/260 persons enriched, 2,146 persons now have a structured active period. Fuseki reloaded — broadcaster triples active.
- [x] **Source attribution** — LLM prompt instructs chatbot to open every answer by naming the source based on context block labels: "According to the Beeld & Geluid Wiki, …" / "According to the Media Suite knowledge graph, …" / "According to the Media Suite documentation, …". Structured wiki lists (`[Gestructureerde data]`) must be presented as complete lists without summarising.
- [x] **`productions_by_broadcaster` routing** — wiki router detects 22 known broadcaster names (VPRO, VARA, NOS, AVRO, …) in questions and routes to `productions_by_broadcaster` SPARQL; generic broadcaster signals route to `all_broadcasters`. "Which productions did VPRO broadcast?" now returns 935 SPARQL hits.
- [x] **Wiki path evaluation** (`eval_wiki.py`) — 8/8 questions passing (100%): period SPARQL (1970s, 1980s), function+period SPARQL (presentatoren jaren zeventig), biographical semantic (Mies Bouwman, Joris Ivens, Frits Thors), broadcaster SPARQL (VPRO, 935 productions), no-attribution check for documentation questions. Known issue: LLM non-deterministically picks which names to show from long SPARQL result lists — routing and retrieval are correct, this is an LLM compliance gap.
- [x] **History-echo bug fix** — when context had no clear answer the LLM fell back on previous assistant turns in conversation history. Fixed by adding explicit instruction to `_USER_PROMPT_TEMPLATE`: history may be used only to resolve references ("it", "the tool"), not to answer the current question.
- [x] **Wiki topic guard** (`wiki_client._is_wiki_relevant()`) — checks for Dutch media signals (years/decades, biographical phrases, function terms, broadcaster names) before calling the wiki API; returns empty immediately if none match. Eliminates spurious semantic matches for off-topic questions (e.g. weather, non-Dutch events). 15/16 test cases correct; year-only false positives are handled downstream by min\_score and LLM instruction.
...
- [ ] **Eliminate tool_entities sync requirement** — `media-suite-learn-chatbot/config.yaml` duplicates `tool_entities` from this repo; adding a new entity requires updating both. Fix: chatbot reads `tool_entities` and `collection_entities` directly from the knowledge base `config.yaml` at startup, or a shared config is published. Confirmed pain point: adding `Ask Media Suite` entity in the KB config did not fix chatbot routing until the chatbot config was also updated.
