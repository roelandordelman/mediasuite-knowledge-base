...
All content originates from the public GitHub repo
https://github.com/beeldengeluid/mediasuite-website (Jekyll/Siteleaf).
...
- **GitHub Issues** from beeldengeluid/mediasuite-website — real researcher
  questions and answers, excellent authentic FAQ material
- **Research publications via DOIs** — papers that used the Media Suite as a
  research tool; selectively ingest methodology sections and generate summaries
  of how the Media Suite was used
- **Jupyter notebooks** — markdown cells from mediasuite example notebooks
- **Data platform documentation** — dataset and collection descriptions from
  data.beeldengeluid.nl
- **Workshop and tutorial materials** — slides and exercises from DH workshops
  where the Media Suite was used
...
│
├── sources/
│   └── mediasuite/                   # future: sources/clariah-core/, sources/publications/
│
├── pipelines/
│   ├── ingest/
│   │   ├── ingest_mediasuite.py      # Parses mediasuite-website repo → JSON chunks
│   │   ├── ingest_github_issues.py   # (planned) GitHub Issues → JSON chunks
│   │   └── ingest_publications.py    # (planned) DOIs → PDFs → filtered chunks
│   │
│   ├── embed/
│   │   └── build_index.py            # JSON chunks → embeddings → ChromaDB
│   │
│   └── graph/                        # (planned) entity extraction → RDF → Fuseki
│       ├── extract_entities.py
│       └── build_graph.py
│
├── stores/
│   ├── chroma_db/                    # gitignored — regenerate via build_index.py
│   └── fuseki/                       # gitignored — planned triplestore
│
├── evaluate/                         # evaluation scripts (add from day one)
│   └── eval_retrieval.py             # (planned) precision/recall on test questions
│
├── knowledge_base.json               # gitignored — generated by ingest pipeline
├── config.yaml                       # chunk sizes, model names, source paths
├── requirements.txt
├── CLAUDE.md                         # this file
└── README.md
```
